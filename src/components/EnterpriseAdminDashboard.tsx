...
export const EnterpriseAdminDashboard: React.FC<EnterpriseAdminDashboardProps> = ({
  ...
  const leadTech = technicianList.find(t => t.name.includes('Rjay') || t.id === 'tech-1') || technicianList[0] || {
    id: 'tech-1',
    name: 'Rjay Picar',
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto overflow-x-hidden scroll-smooth no-scrollbar p-3.5 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="w-full max-w-[1600px] mx-auto space-y-5 sm:space-y-6 lg:space-y-7 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 relative">
            ...
            {/* Windows Push Notification Authorization Pill */}
            {notificationPermission === 'default' && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-full text-xs font-bold shadow-2xs transition cursor-pointer shrink-0"
                title="Enable Windows desktop popups when technicians arrive"
              >
                <Bell className="w-3.5 h-3.5 text-amber-600 animate-bounce" />
                <span className="hidden xl:inline">Enable Windows Alerts</span>
                ...
                <div className="flex items-start sm:items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      Physical inspection photos from official installation tasks for {project.name}. Click to inspect full resolution.
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs font-mono font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-xl border border-slate-200/60">
              ...
              {filteredEvidenceTasks.length === 0 ? (
                <div className="p-8 rounded-2xl bg-slate-50 border border-slate-200/70 text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-500 max-w-md mx-auto">
                    Upload photos in task evidence modal to attach visual inspection records.
                  </p>
                      ...
                      >
                        ...
                        <div className="px-3 py-2 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-[10px] font-mono text-slate-500">
                          <div className="flex items-center gap-1 truncate mr-1">
                            ...
                            <span className="truncate">{task.owner || project.teamLead}</span>
                          </div>
